...
from lib.util import AveragePrecisionMeter, Warp
...
class MultiLabelMAPEngine(Engine):
    ...
    def on_end_epoch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        pc, rc, fc, po, ro, fo = self.state['ap_meter'].metrics()
        loss = self.state['meter_loss'].value()[0]
        if display:
            if training:
                # print(model.module.spatial_pooling)
                ...
                print('Test: \t Loss {loss:.4f}\t mAP {map:.3f}\n\t'
                'P-C {pc:.3f}\t R-C {rc:.3f}\t F1-C {fc:.3f}\t'
                'P-O {po:.3f}\t R-O {ro:.3f}\t F1-O {fo:.3f}'.format(loss=loss, map=map, pc=pc, rc=rc, fc=fc, po=po, ro=ro, fo=fo))
    ...
    def on_start_batch_new(self, training, model, criterion, data_loader, optimizer=None, display=True):
