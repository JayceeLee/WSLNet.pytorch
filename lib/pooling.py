...
class ClassWisePoolFunction(Function):
    ...
    def forward(self, input):
        ...
        output = torch.max(x, 2)[0]
        self.save_for_backward(input)
        # return output.view(batch_size, num_outputs, h, w) / self.num_maps
        return output.view(batch_size, num_outputs, h, w)
...
class ClassWisePoolFunctionAvg(Function):
    def __init__(self, num_maps):
        super(ClassWisePoolFunctionAvg, self).__init__()
        self.num_maps = num_maps
    ...
    def forward(self, input):
        ...
        if num_channels % self.num_maps != 0:
            print('Error in ClassWisePoolFunctionAvg. The number of channels has to be a multiple of the number of maps per class')
            sys.exit(-1)
...
class ClassWisePool_avg(nn.Module):
    def __init__(self, num_maps):
        super(ClassWisePool_avg, self).__init__()
        self.num_maps = num_maps

    def forward(self, input):
        return ClassWisePoolFunctionAvg(self.num_maps)(input)
