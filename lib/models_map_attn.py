...
class ResNetWSL(nn.Module):
    ...
    def __init__(self, model, num_classes, num_maps, pooling=None):
        ...
        self.attnlayer_map = AttentionLayer(num_classes, num_maps)
    ...
    def get_config_optim(self, lr, lrp):
        return [{'params': self.features.parameters(), 'lr': lr * lrp},
                ...
                {'params': self.attnlayer_map.parameters()},
                {'params': self.class_pooling.parameters()},
                {'params': self.class_pooling_avg.parameters()},
                {'params': self.spatial_pooling.parameters()}]

def resnet50_map_attn(num_classes, pretrained=True, kmax=1, kmin=None, alpha=1, num_maps=1):
...
def resnet101_map_attn(num_classes, pretrained=True, kmax=1, kmin=None, alpha=1, num_maps=1):
